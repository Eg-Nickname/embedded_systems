...
void save_to_file(std::vector<uint32_t> subtasks,
                  std::vector<std::unordered_map<uint32_t, uint32_t>> adj,
...
void gen_file_structure(int32_t tc, int32_t pp, int32_t hc, int32_t cl,
    ...
    // Generate how many subtasks each task has
    for (auto i = 1; i < subtasks.size(); i++) {
        if (subtask_chance_dist(gen) < DistConfig::TASK_SPLIT_CHANCE) {
            subtasks[i] = subtask_count_dist(gen);
        }
    }
