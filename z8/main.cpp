...
// Chance that task is split
constexpr uint32_t TASK_SPLIT_CHANCE = 40;

// All_tasks * fration = how many subtask
constexpr uint32_t MAX_SUBTASK_FRAC = 20;

// Chance of task being conditional
constexpr uint32_t CONDTIONAL_TASK_CHANCE = 40;
...
using cond_t = std::string;
using edgew_t = uint32_t;
using edge_t = std::pair<uint32_t, std::optional<cond_t>>;

std::vector<std::unordered_map<uint32_t, edge_t>> gen_dag(uint32_t node_count,
                                                          bool random_edge_w);

void gen_file_structure(int32_t tc, int32_t pp, int32_t hc, int32_t cl,
                        bool random_edge_w, std::string& output_file);

void save_to_file(std::vector<uint32_t> subtasks,
                  std::vector<std::unordered_map<uint32_t, edge_t>> adj,
                  std::vector<std::vector<uint32_t>>& proc,
                  std::vector<std::vector<std::vector<int32_t>>>& times,
                  std::vector<std::vector<std::vector<int32_t>>>& cost,
...
std::string gen_cond() {
    ...
    std::vector<std::string> conds = {
        "czas < ",         "czas > ",      "zmienna == ",
        "zmienna < ",      "zmienna > ",   "tempteratura > ",
        "tempteratura < ", "cisnienie > ", "cisnienie < ",

    };
    std::uniform_int_distribution<std::uint32_t> cond_dist(0, conds.size() - 1);
    std::uniform_int_distribution<std::uint32_t> time_dist(2, 60);
    std::uniform_int_distribution<std::uint32_t> var_dist(20, 90);
    std::uniform_int_distribution<std::uint32_t> temp_dist(80, 800);
    std::uniform_int_distribution<std::uint32_t> pres_dist(200, 700);
    auto rand_cond = cond_dist(gen);
    switch (rand_cond) {
    case 0:
    case 1:
        return conds[rand_cond] + std::to_string(time_dist(gen)) + "s";
        break;
    case 2:
    case 3:
    case 4:
        return conds[rand_cond] + std::to_string(var_dist(gen));
        break;
    case 5:
    case 6:
        return conds[rand_cond] + std::to_string(temp_dist(gen)) + "K";
        break;
    case 7:
    case 8:
        return conds[rand_cond] + std::to_string(pres_dist(gen)) + "Pa";
        break;
    default:
        break;
    }
...
std::vector<std::unordered_map<uint32_t, edge_t>> gen_dag(uint32_t node_count,
    ...
    // Grpah
    std::vector<std::unordered_map<uint32_t, edge_t>> adj(node_count);
    if (random_edge_w) {
    ...
    for (int32_t n = 1; n < node_count; n++) {
        ...
        // if conditional edge we do sth special
        std::uniform_int_distribution<std::int32_t> conditional_dist(0, 100);
        std::uniform_int_distribution<std::uint32_t> con_edge_dist(0, n - 1);
        if (conditional_dist(gen) < DistConfig::CONDTIONAL_TASK_CHANCE) {
            auto con_node = con_edge_dist(gen);
            adj[con_node][n] = {edge_dist(gen), gen_cond()};
        } else {
            for (int32_t cc = 0; cc < con_count_dist(gen); cc++) {
                auto con_node = con_edge_dist(
                    gen); // if con_node repeat, it will only update
                // edge weight (random and we dont care)
                adj[con_node][n] = {edge_dist(gen), std::nullopt};
            }
        }
    }
    return adj;
...
void gen_file_structure(int32_t tc, int32_t pp, int32_t hc, int32_t cl,
    ...
    //
    std::uniform_int_distribution<std::int32_t> subtask_count_dist(
        2, std::max(static_cast<uint32_t>(2),
                    (DistConfig::MAX_SUBTASK_FRAC * tc) / 100));
    std::uniform_int_distribution<std::int32_t> subtask_chance_dist(0, 100);

    std::vector<uint32_t> subtasks(tc, 1);

    // Gen times for each task on each proc
    std::vector<std::vector<std::vector<int32_t>>> times(
        tc, std::vector<std::vector<int32_t>>(p_count));
    ...
    for (size_t t_idx = 0; t_idx < times.size(); ++t_idx) {
        ...
        for (size_t i = 0; i < p_count; i++) {
            ...
            t[i].resize(subtasks[t_idx]);

            if (i < pp) {
                for (size_t s = 0; s < subtasks[t_idx]; s++) {
                    t[i][s] = pp_times_dist(gen);
                }
            } else {
                for (size_t s = 0; s < subtasks[t_idx]; s++) {
                    t[i][s] = hc_times_dist(gen);
                }
            }
        }

        if (skips == p_count) {
            t[0].resize(subtasks[t_idx]);
            t[0].pop_back();
            for (size_t s = 0; s < subtasks[t_idx]; s++) {
                t[0][s] = pp_times_dist(gen);
            }
        }
    }
    ...
    for (size_t t_idx = 0; t_idx < cost.size(); ++t_idx) {
        auto& c = cost[t_idx];
        for (size_t i = 0; i < p_count; i++) {
            c[i].resize(subtasks[t_idx]);

            if (i < pp) {
                for (size_t s = 0; s < subtasks[t_idx]; s++) {
                    c[i][s] = pp_cost_dist(gen);
                }
            } else {
                for (size_t s = 0; s < subtasks[t_idx]; s++) {
                    c[i][s] = hc_cost_dist(gen);
                }
            }
        }
    }
    ...
    // save to file
    save_to_file(subtasks, adj, proc, times, cost, comms, output_file);
}
...
void save_to_file(std::vector<uint32_t> subtasks,
    ...
    for (size_t task_id = 0; task_id < adj.size(); task_id++) {
        // Row consists of task id | succesor_count | adj list of
        // succesorts with weights
        file << task_id << " " << adj[task_id].size();
        // List all tasks
        for (auto& [succesor, e] : adj[task_id]) {
            auto& [weight, cond] = e;
            file << " " << succesor << "(" << weight
                 << ")"; // insert space before each entry to spearate
            // them
            // add subtasks if task has them
            if (cond) {
                file << "[" << cond.value() << "]";
            }

            if (subtasks[succesor] > 1) {
                file << "{" << subtasks[succesor] << "}";
            }
        }
        file << std::endl;
    }
    ...
    for (auto& t_row : times) {
        for (auto& t : t_row) {
            ...
            if (t.size() == 1) {
                ...
                file << "{";
                for (auto sbt_idx = 0; sbt_idx < t.size(); sbt_idx++) {
                    file << t[sbt_idx];
                    if (sbt_idx < t.size() - 1) {
                        file << ", ";
                    }
    ...
    for (auto& c_row : cost) {
        for (auto& c : c_row) {
            if (c.size() == 1) {
                ...
                file << "{";
                for (auto sbc_idx = 0; sbc_idx < c.size(); sbc_idx++) {
                    file << c[sbc_idx];
                    if (sbc_idx < c.size() - 1) {
                        file << ", ";
                    }
